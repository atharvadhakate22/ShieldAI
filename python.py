...
from prevention import apply_prevention, load_blocklist, monitor_block_list, ip_block_check
...
def extract_features_and_predict(packet):
    ...
    if IP in packet:
        ...
        load_blocklist()  # Refresh blocklist periodically or as needed
        threading.Thread(target=unblock_expired_ips, daemon=True).start()
        firewall_loop()

        if ip_block_check(src):
            # Drop packet processing early if source is blocked
            return
